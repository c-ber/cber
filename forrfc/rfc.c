...
int rfc_phase_init(uint16_t u_ifgroup)
{
    ...
    //pst_commit->ul_free_cell_num = ul_rfc_free_cell_num;
    return ACL_OK;
}
...
#define DATA64_BITMASK_SET(pud_mask,n)\
	{\
		...
		int  _length64 = LENGTH64 -1;\
		if ( (n & (_length64)) < 32)\
		{\
		    __mask32[0] = 0;\
			__mask32[1] = 1 << (n & (_length64));\
		}\
		else \
		{\
			__mask32[0] = 1 << ((n & (_length64)) - 32 );\
			__mask32[1] = 0;\
		}\
	}
...
int rfc_phase0(rfc_phase_set_s *pst_acl_commit, rfc_filter_set_s *pst_filter_set, rfc_filter_index_s *pst_filter_index)
{
    ...
    rfc_filter_action_s *pst_filter_action[MPP_MAX_PHASE0_CELL] = {NULL};
    rfc_filter_action_s * pst_action_temp;
    ...
    for (com = 0; com < 7; com++)
    {
        ...
        {
            MC_PRINT("1  cell alloc failure in table %d, need is %d, free is %d\n", com, MPP_MAX_PHASE0_CELL, pst_acl_commit->ul_free_cell_num);
            return ACL_CELL_ERROR;
        }
...
int  rfc_phase(rfc_pnoder_s *pst_node1, rfc_pnoder_s *pst_node2, rfc_pnoder_s *pst_node3, uint32_t ul_table)
{
    ...
    {
        MC_PRINT("2  cell alloc failure in table %d, need is %d, free is %d\n", ul_table, ul_cell_num, pst_acl_commit->ul_free_cell_num);
        return ACL_CELL_ERROR;
    }
...
int rfc_phase3(rfc_pnoder_s *pst_prev_phase_nodes, rfc_pnoder_s *pst_phase_nodes, uint16_t u_ifgroup)
{
    ...
    {
        MC_PRINT("3  cell alloc failure in table %u, need is %u, free is %u\n", 12, ul_cell_num, pst_acl_commit->ul_free_cell_num);
        return ACL_CELL_ERROR;
    }
    ...
    {
        pud_bmp1 = &pst_acl_phase_data->ud_cbm_data[pst_ces1[i].ul_cbm_index];
        //printf("%s.%d: pud_bmp1[0]:%#lx\n", __func__, __LINE__, pud_bmp1[0]);
        
        for (j = 0; j < pst_node2->ul_eq_id; j++)
        {
            pud_bmp2 = &pst_acl_phase_data->ud_cbm_data[pst_ces2[j].ul_cbm_index];
            //printf("%s.%d: pud_bmp2[0]:%#lx\n", __func__, __LINE__, pud_bmp2[0]);
            
            for (m = 0; m < pst_acl_phase_data->u_real_size; m++)
                ud_bmp[m] = pud_bmp1[m] & pud_bmp2[m];
            //printf("%s.%d: ud_bmp[0]:%#lx\n", __func__, __LINE__, ud_bmp[0]);
                
            ul_cost = rfc_get_rule_cost(ud_bmp);
            //printf("%s.%d: ul_cost=%d\n", __func__, __LINE__, ul_cost);
...
//#define MASK_RULE_DBG


#define MASK_RULE_PRINTF_DEBUG(fmt, arg...) printf(fmt , ##arg)
...
{
    int rv = 0;
    npcp_fct_commit_st fct_commit;
    memset(&fct_commit, 0, sizeof(npcp_fct_commit_st));
    fct_commit.total_len = total_len;
    fct_commit.offset    = offset;
    fct_commit.len       = len;
    fct_commit.num       = num;
    fct_commit.over      = over;
    fct_commit.iifgrp    = iifgrp;
    fct_commit.crc       = crc;
    memcpy(fct_commit.buff, ptr, len);

//    rv = npcp_cmd_exec(NPCP_MID_MASK, NPCP_CMD_MASK_RULE_COMMIT,
//            sizeof(npcp_fct_commit_st), &fct_commit, NULL, NULL);
    return rv;
}
...
//    int rv;
//
//    rv = npcp_cmd_exec(NPCP_MID_MASK, NPCP_CMD_MASK_RULE_INIT,
//            0, NULL, NULL, NULL);
//    return rv;
}
...
int mask_rule_fct_commit(uint8_t *fct, uint32_t size, uint8_t iifgrp)
{
    ...
    uint32_t offset = 0;
    uint16_t len = 0;
    uint32_t num = 0; /* transfer times */
    uint8_t over = 0; /* transfer over flag, 1 for over */
    uint8_t crc  = 0; /* transfer over,take with crc */
    int rv;
    uint8_t *ptr = fct;

    if (fct == NULL)
    {
        MASK_RULE_PRINTF_DEBUG("fct == NULL\n");
        return XEINVAL;
    }

    /* size must 8 bytes align */
    if ( size % 8 )
    {
        //printf("size % 8 = %d\n", size % 8);
        return XEINVAL;
    ...
    MASK_RULE_PRINTF_DEBUG("l = %d\n", l);
    while ( l )
    {
        if ( l / SINGLE_SEND_LEN )
        {
            len = SINGLE_SEND_LEN;
        }
        else
        {
            len = l % SINGLE_SEND_LEN;
        }

        /* for over, set over flag */
        if ( l <= SINGLE_SEND_LEN )
        {
            over = 1;
            crc = CRC8(fct, size);
            //MASK_RULE_PRINTF_DEBUG("crc %d\n", crc);
        }

        num++;
        MASK_RULE_PRINTF_DEBUG("offset:%8d, left:%8d, len:%d, num:%5d, over:%d\n",
                offset, l, len, num, over);
        rv = npcp_mask_rule_commit(ptr, size, offset, len, num, over, iifgrp, crc);
        if (rv)
        {
            MASK_RULE_PRINTF_DEBUG("rv = %d\n", rv);
            return rv;
        }
...
int rfc_phase_commit(void)
{
    ...
    rfc_filter_set_s    *pst_backup_set = NULL;
    rfc_filter_index_s  *pst_filter_index = g_ix_rfc_table_base->st_filter_index;
    ...
    for (i = 0; i < MPP_MAX_ACL_NUM; i++)
    {
        if (pst_filter_index[i].u_filter_num > 0)
        {
            if (pst_filter_index[i].u_changed == ACL_CHANGED)
            {
                ...
                //rfc_phase0 ,里面至少循环7*65536次
                if ((result = rfc_phase0(pst_acl_commit,
                        pst_filter_set, &pst_filter_index[i])) != ACL_OK)
                {
                    return result;
                }
                ...
                if ((result = rfc_phase3(&pst_acl_commit->st_phase_nodes[10],
                        &pst_acl_commit->st_phase_nodes[12], i)) != ACL_OK)
                {
                    return result;
                }
                ...
                MPP_GET_MD_VAR(g_ix_rfc_table_base->pst_set) [i] =
                        ADDR_MDU_TO_MD(pst_filter_set);

                (MPP_GET_MDU_VAR(g_ix_rfc_table_base->pst_acl_ptr) [i]) =
                        (rfc_phase_set_s *)(pst_acl_commit);
                (MPP_GET_MD_VAR(g_ix_rfc_table_base->pst_acl_ptr) [i]) =
                        (rfc_phase_set_s *) ADDR_MDU_TO_MD(pst_acl_commit);

//这一段是用于npcp传输
//                rv = mask_rule_fct_commit((uint8_t *)pst_acl_commit,
//                        sizeof(rfc_phase_set_s), i + 1);
//                if (rv)
//                {
...
//                }
            ...
            {
                MPP_GET_MD_VAR(g_ix_rfc_table_base->pst_set) [i] =
                        (rfc_filter_set_s *) ADDR_MDU_TO_MD(pst_filter_set);
            }
        }
    ...
    for (i = 0; i < MPP_MAX_FILTERS; i++)
    {
        if (pst_filter_set->st_filter[i].b_newitem == ACL_NEW)
        {
            ...
            {
                pst_backup_set->st_filter[i].ud_filter[j] =
                        pst_filter_set->st_filter[i].ud_filter[j];
            }
            
            // assign the statistics
            pst_filter_set->st_filter[i].ud_statistics +=
                    pst_backup_set->st_filter[i].ud_statistics;
...
int rfc_phase_commit_not_submit(void)
{
    ...
    uint16_t u_ifgroup;
    rfc_phase_set_s     *pst_acl_commit = g_ix_rfc_table_base->pst_acl_commit;
    rfc_phase_set_s     *pst_acl_temp;
    rfc_filter_set_s    *pst_filter_set = g_ix_rfc_table_base->pst_backup_set, *pst_backup_set;
    rfc_filter_index_s  *pst_filter_index = g_ix_rfc_table_base->st_filter_index;
    ...
    for (i = 0; i < MPP_MAX_ACL_NUM; i++)
    {
        if (pst_filter_index[i].u_filter_num > 0)
        {
            if (pst_filter_index[i].u_changed == ACL_CHANGED)
            {
                ...
                pst_acl_commit->ul_enabled = ENABLE;
                pst_acl_temp = (rfc_phase_set_s *) MPP_GET_MDU_VAR(
                        g_ix_rfc_table_base->pst_acl_ptr) [i];
...
int rfc_ptr_init ()
{
    ...
    g_ix_rfc_table_base->pst_acl_commit = g_ix_rfc_table_base->st_phase + MPP_MAX_ACL_NUM;
