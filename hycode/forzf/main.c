...
void init_pcap()
{
    ...
    //写头
    fwrite( hexData, 24, 1, fp );

    fclose(fp);
}
void save_to_pcap(const unsigned char * raw_pkt, uint32_t len)
{
    unsigned char hexData[8] = {
        0x24, 0x95, 0xB5, 0x4F, 0x62, 0xB8, 0x06, 0x00
    };

    FILE *fp = NULL;
    fp = fopen("rcv.pcap", "ab+");
    if( NULL == fp )
    {
        return;
    }
    //写时间
    fwrite( hexData, 8, 1, fp );
    //写两个长度
    fwrite( &len, 4, 1, fp );
    fwrite( &len, 4, 1, fp );
    //写报文
    fwrite( raw_pkt, len, 1, fp );
...
void decode(const struct pcap_pkthdr * pkthdr, const unsigned char *raw_pkt)
{
    ...
    if (payload[0] == 0x64 && payload[1] == 0x70 && payload[2] == 0x69)
    {
        ...
        printf("\n");
        snd_tcp_data(&tcp_fd);
        save_to_pcap(raw_pkt, pkthdr->len);
    }
