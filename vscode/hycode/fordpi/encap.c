...
extern uint16_t kr_ipd_port_list[12];
extern uint8_t  kr_round_robin;
uint32_t dpi_packet_xmit(cvmx_wqe_t *work)
{
//    int port = kr_ipd_port_list[(cvmx_get_core_num() + (kr_round_robin++)) % 12];;
//    int queue =  cvmx_pko_get_base_queue(port);
    int temp = 0;
//
//    cvmx_pko3_pdesc_t   desc;
//
//    cvmx_pko3_pdesc_from_wqe(&desc, (void *) work, 0);
//
//    temp = cvmx_pko3_pdesc_transmit(&desc, queue);
    return temp;
}

void dpi_packet_free(cvmx_wqe_t *work)
{
...
//    oifgrp_st  *oifgrp_entry    = NULL;
//    oifgrp_entry                = ifgrp_get_oifgrp(oifgrp_id);


//    if (octeon_has_feature(OCTEON_FEATURE_CN78XX_WQE))
//    {
//        work = cvmx_wqe_soft_create(snd_data, snd_len);
//        if(NULL == work)
//        {
//            return MP_WORK_NULL;
//        }
//        ((cvmx_wqe_78xx_t *)(work))->word2.software = 0;
//
//        dpi_packet_xmit(work);
//        dpi_packet_free(work);
//
...
mp_code_t packet_identity_result_process(dpi_result_t * rlt)
{
    ...
    //发送报文
    printf("packet_identity_result_send , osi.plen = %#x\n", osi.plen);
    packet_identity_result_send(1, pbuf, osi.plen);
    printf_buf(pbuf, osi.plen);
